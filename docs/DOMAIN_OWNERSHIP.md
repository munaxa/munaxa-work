...
| Custody transfer, acknowledgement, cancellation, correction, asset incident, liability, waiver, clearance projection | — | not owned yet; `assets` will own them in Phase 5.3 Checkpoints 3–4. **Deliberately unbuilt**: transfer needs D-5.3-08, correction D-5.3-10, the condition scale D-5.3-05, and the clearance projection D-5.3-03 if a deduction is in scope. `issued`, `in_custody` and `returned` are **not** asset statuses — the current holder is derived from the open custody row (ADR-0070, D-5.2-16) |
