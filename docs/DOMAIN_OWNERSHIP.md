...
| Offboarding clearance contribution (what Assets contributes: outstanding custody as a blocker) | `assets` | Phase 5.3 · Checkpoint 4 ✅ — **derived on every read; no clearance state is persisted anywhere.** Assets publishes `assetsClear` and the blocking custodies; it does **not** own the clearance decision, which is Offboarding's (Phase 11.2) across domains Assets cannot see. Under approved D-5.3-01(a) an open custody is outstanding whether or not the employment has ended, and nothing closes it automatically. **The AD-006 waiver is not built** |
