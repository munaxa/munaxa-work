...
**Status: approved. Implementation proceeded on the decisions recorded in §0.** This document is
what `06_PHASE_5_EMPLOYMENT.md` §51 and `27_DEVELOPMENT_PROTOCOL.md` Step 2 require before
implementation begins: the analysis, the plan, the risks, and the ambiguities that needed a
decision before a line was written.

Twelve ambiguities were raised in §12. Four were answered directly by the approver and are recorded
in §0; the remaining eight were resolved within the approved constraints and are annotated in §12
with the decision taken and why none of them needed a second approval.

---

## 0. Approved decisions

Recorded verbatim in effect, and binding over any contrary wording elsewhere in the phase prompts.

### A1 — Employee number: **generated, tenant-scoped**

Generated by Munaxa Work; unique within the tenant; immutable after creation; never reused after an
employment ends; **no arbitrary caller override**. It is not the person's identity, and neither the
national identifier nor the person id may be used as one.

Migration and import supply a **separate legacy/external employee identifier** rather than
overriding generation. No change to the completed Organization module to hold number configuration
— the mechanism is owned by Employment.

The repository has **no existing sequence or counter convention** (verified: no sequence table, no
counter, nothing in the kernel). So the smallest Employment-scoped mechanism consistent with the
existing database architecture is implemented — a tenant-scoped counter row allocated inside the
same transaction — and a reusable cross-cutting sequence capability is recorded as technical debt
with an ADR rather than built here.

### A2 — Lifecycle: **DRAFT → PENDING_APPROVAL → ACTIVE → SUSPENDED → ENDED**

No `ON_LEAVE` — leave status belongs to the Leave domain and is not duplicated here. No `RETIRED`
and no `ARCHIVED` as employment statuses. An ended employment carries a **structured end reason**,
tenant-supplied, with no country-specific reasons hardcoded. The lifecycle stays compatible with a
future Offboarding domain, which will orchestrate the exit while Employment remains the
authoritative owner of the relationship and its final state.

This matches the in-repo approved v1.0 spec's `PENDING_APPROVAL` and `ENDED` conventions rather than
introducing a conflicting enum.

### A6 — Work location: **not implemented, extension point preserved**

An organizational unit and a physical work location are **semantically different**, and no false
relationship is created to satisfy the prompt. No location entity is added to the completed
Organization module, Phase 3 is not modified, and no duplicate location hierarchy is created inside
Employment. The repository contains no model that genuinely represents a physical work location, so
the concept is **absent** from this phase, the extension point is preserved and named, and the gap
is recorded for a future Organization/Workforce Location decision.

### A9 — Idempotency: **database uniqueness and natural-key protection only**

No generic idempotency infrastructure is introduced inside Employment. Duplicate employment creation
**fails deterministically** rather than silently creating a second employment. Full request-level
idempotency is **not claimed**, and generic request idempotency is recorded as technical debt for a
future platform or kernel capability that Employment can later adopt through its published contract.

### Standing constraints

Completed Phase 0–4 architecture is **not modified** to force any of these requirements, and no
future domain — Leave, Offboarding, Workforce Relations, Documents, Loans, Health & Claims, Country
Packs, Government Integrations, Mobile, AI — is implemented.
...
## 12. Ambiguities, and how each was resolved

A1, A2, A6 and A9 were **answered by the approver** and are recorded verbatim in §0. The remaining
eight were resolved inside those constraints. None of the eight changes business behaviour in a way
the approved decisions and the existing approved architecture did not already determine, so none
required a second approval — the reasoning for each is stated so that judgement can be checked
rather than taken on trust.

### A1 — Employee number → **decided, §0**

Generated, tenant-scoped, immutable, never reused, no caller override, with a separate
`external_employee_number` for migration. No existing sequence convention was found, so an
Employment-owned counter is implemented and a reusable sequence capability is recorded as debt.

### A2 — Lifecycle → **decided, §0**

`draft → pending_approval → active → suspended → ended`, with a structured tenant-supplied end
reason. No `on_leave`, no `retired`, no `archived`.

### A3 — Concurrent employments → **resolved: schema supports many, at most one open**

Determined by two approved statements read together. AD-004 requires the *architecture* to support
historical, concurrent and future employments; the same document marks concurrent employments
"(future)"; and §0's A9 requires that **duplicate employment creation fail deterministically rather
than silently creating a second employment**.

With numbers generated rather than supplied, a unique number cannot catch a duplicate create — every
retry earns a fresh number. The natural key that *can* is the person: a second open employment for
somebody who already has one is exactly the duplicate A9 names. So:

- the table carries no uniqueness on `person_id`, and holds any number of employments per person —
  rehire, history, and the concurrent case AD-004 requires the architecture to support;
- a **second open (non-`ended`) employment for the same person is refused**, in the domain and by a
  partial unique index on `(tenant_id, person_id) where status <> 'ended'`, which is what makes the
  refusal deterministic under concurrency rather than merely likely;
- enabling genuine concurrency later is dropping that index, with no reshaping of data.

### A4 — Contracts and probation → **resolved: implemented, as records only**

They are in the in-repo approved v1.0 aggregate, and §0 directs that approved v1.0 conventions be
preserved rather than contradicted. A contract *record* — term dates, probation end and outcome,
notice period, working hours, a document *reference* — is not a document management system (§24) and
carries no money and no statutory computation. `notice_period_days` is a recorded term, never a
computed statutory one; that is Phase 11.1's.

### A5 — Managers → **resolved: one open primary line, `line_type` as the extension point**

§16 requires the relationship, its changes and its history, and is silent on multiplicity. Inventing
dotted-line semantics would be inventing a business rule where the specification is silent (§1). So
exactly **one open `primary` reporting line** is enforced, in the domain and by partial unique index,
and `line_type` exists as the column a future functional or matrix line is added through without a
reshape.

### A6 — Work location → **decided, §0: not implemented**

No column, no entity, no false relationship to `unit_id`. The extension point is named in the module
document and the ADR: a future `work_location_id` on `employment_assignment`, once an authoritative
model exists in Organization. Recorded as an explicit architectural gap.

### A7 — Filled-headcount adapter → **resolved: supplied**

Phase 3 designed the port for this and its own adapter's documentation says so; the Phase 4 debt
register lists the always-zero filled count as "must be addressed: Phase 5". Supplying it changes no
Phase 3 code — `NoAssignmentsYet` stays exactly where it is, and the composition root chooses
Employment's adapter instead. That is the seam being used as designed, not a completed module being
modified.

### A8 — Compensation and benefit references → **resolved: omitted**

Compensation is Phase 10 and Benefits is Phase 12, both non-goals, and §0 forbids implementing future
domains. An empty reference table nothing writes to is the fake completeness §50 forbids.

### A9 — Idempotency → **decided, §0**

Uniqueness constraints and natural-key protection; deterministic failure on duplicate creation; no
claim of request-level idempotency; the generic capability recorded as debt.

### A10 — Admin UI → **resolved: read-only**

Consistent with Phases 3 and 4, whose write screens are already recorded as debt against Phase 18/19.
§41 lists the screens Employment must show and does not require them to write. Building write screens
only here would make Employment the sole module with them and pre-empt Phase 18/19's design.

### A11 — Person names on the employment list → **resolved: bounded per-page resolution, no change to People**

§0 forbids modifying completed Phase 0–4 architecture, which rules out adding a batched query to
`people`. So Employment declares a `PersonDirectoryPort` and resolves names **through People's
published query**, once per row of a single page — bounded by the page size, never by the size of the
register. The cost is measured rather than assumed, and the batched read that would remove it is
recorded as debt against the same phase that owns the projection store.

### A12 — Foreign key to `person` → **resolved: kept, and it is the only cross-module key**

§37 requires that an employment reference an existing Person and §36 asks for explicit foreign keys.
`employment.person_id` carries one. It is not the coupling `identity` avoided: that reference points
*forward* to a module Phase 2 could not depend on, whereas this one points *backward* to a module
Employment already depends on, in the same schema and the same tenant, enforcing Employment's own
invariant.

The organizational references do **not** carry foreign keys, for a reason that is worth stating
plainly rather than leaving implicit — and it exposes a real gap:

- A same-tenant composite foreign key to `job_position` or `financial_center` would need a new unique
  index **on Organization's tables**, which §0 forbids.
- Organization publishes no single-entity read for a position or a cost centre — `list-positions` is
  paged and filtered, and centres have no query at all — so Employment cannot verify those references
  through the published contract either.

So a **unit** reference is verified through `organization.unit-ancestry`, which is published and
answers not-found for a unit that does not exist in the caller's tenant; **position** and **cost
centre** references are stored with tenant containment resting on row-level security, which makes
another tenant's row unreadable rather than unreferenceable. The missing single-entity reads are
recorded as a gap against Organization's contract, not worked around by reaching into its tables.

---

## ADRs written for this phase

| ADR | Subject |
| --- | ------- |
| ADR-0039 | Employment number: generated, tenant-scoped, immutable, never reused (A1) |
| ADR-0040 | Employment lifecycle, and why "on leave" belongs to Leave (A2) |
| ADR-0041 | Work location is not modelled, and the extension point that is preserved (A6) |
| ADR-0042 | How Employment references another module: one foreign key, one published query, and one gap (A11, A12) |

---

## What happened next

Schema and migration → domain → application → infrastructure → API → admin screens → tests at every
layer → `pnpm verify` → performance measurement against a seeded database → documentation, ADRs,
module guide, ER diagram, release notes, debt register → completion report → stop.

The completion report is [`phase-5-report.md`](phase-5-report.md).
