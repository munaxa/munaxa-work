 ...
 * `scope`, `roles` and `perms`. A field this interface does not declare is a field this
 * repository cannot accidentally start trusting, and `tid` is the one that matters: declaring
 * it would put a tenant chosen by the token within reach of tenant resolution.
 *
 ...
 */
const principalFrom = (claims: VerifiedAccessToken): PlatformPrincipal | undefined => {
...
export class PlatformTokenAuthenticationPort implements PlatformAuthenticationPort {
  public constructor(private readonly verifier: AccessTokenVerifier) {}
  ...
  private principalOf(
    ...
    try {
      return principalFrom(this.verifier.verifyAccessToken(token));
    } catch {
