import ar from '@work/career/locales/ar.json';
import en from '@work/career/locales/en.json';

/**
 * The career screen's text, in both first-class languages.
 *
 ...
 *
 * A *code* is never translated here. A path code, a pool code and a readiness level's code are
 * tenant values, so the screen renders what the customer stored rather than looking it up in a list
 * this product ships. A **status**, a **kind** and a **category** are different: those are this
 * module's own closed vocabularies, and somebody reading `stretch_assignment` in English on an
 * Arabic succession screen is a translation this product owes them.
 */
 ...
 * choice and survives review, whereas `career.label.succession` on the screen is unmistakably a
