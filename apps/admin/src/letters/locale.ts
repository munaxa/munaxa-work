import ar from '@work/letters/locales/ar.json';
import en from '@work/letters/locales/en.json';

/**
 * The letters screens' text, in both first-class languages.
 *
 ...
 *
 * A template's *code* and *category* are never translated here: they are tenant values, and the
 * screen renders what the customer stored. A **status**, an **approval state** and a **signature
 * state** are this module's own closed vocabularies and are translated — an HR administrator
 * reading `pending_approval` in English on an Arabic screen is a translation this product owes
 * them.
 *
 * A letter's **body** is never translated either, for a different and stronger reason: it is what
 * the letter said, frozen at issue. Rendering anything but the stored text would make the screen
 * disagree with the document somebody is holding.
 */
 ...
 * choice and survives review, whereas `letters.label.register` on the screen is unmistakably a
