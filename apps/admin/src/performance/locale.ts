import ar from '@work/performance/locales/ar.json';
import en from '@work/performance/locales/en.json';

/**
 * The performance screens' text, in both first-class languages.
 *
 ...
 *
 * A *code* is never translated here. A cycle code, a competency code and a goal category code are
 * tenant values, so the screen renders what the customer stored rather than looking it up in a list
 * this product ships. A **status**, a **rating outcome** and an **exclusion reason** are different:
 * those are this module's own closed vocabularies, and somebody reading `not_applicable` in English
 * on an Arabic screen is a translation this product owes them.
 */
