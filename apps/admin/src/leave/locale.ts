import ar from '@work/leave/locales/ar.json';
import en from '@work/leave/locales/en.json';

/**
 * The leave screens' text, in both first-class languages.
 *
 ...
 *
 * A *code* is never translated here. A leave-type code, a paid-treatment code, a reason code and a
 * statutory source are tenant or country-pack values (00B), so the screen renders what the customer
 * stored rather than looking it up in a list this product ships — the same reason the domain
 * refuses to interpret them.
 */
 ...
 * choice and survives review, whereas `leave.label.balances` on the screen is unmistakably a
