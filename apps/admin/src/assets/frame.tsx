...
/**
 * The shape every assets screen is drawn in.
 *
 * Nothing here holds a fact. These are the six decisions the seven completed slices settled —
 ...
 *
 * An asset tag, a serial number, a civil date or an identifier sitting inside an Arabic sentence is
...
/**
 * A count of days, isolated **and forced left to right**.
 *
 * `<bdi>` alone is not enough where a neutral character sits beside the digits. The Leave slice
 * found this on a signed balance and the Attendance slice on a negative duration — `480-` rendering
 * as `480-` reversed into nonsense. Every day-count on these screens goes through here, not only
 * the ones that look at risk, because a column whose digit order depended on the value would be a
 * column nobody could scan.
 */
 ...
 *
 * An issue note, a return note, a description, a location note and a purchase reference are written
 * by people in whichever language they chose. An English sentence rendered bare inside an Arabic
 * table has its trailing full stop moved to the front, because the punctuation is neutral and takes
 * the paragraph's direction. `<bdi>` infers the run's own direction from its first strong character.
 */
...
/** A term from one of Assets' closed vocabularies, translated, with an optional tone. */
export const Term = ({
  t,
  group,
  value,
  ...
  value === undefined ? (
    ...
    <Badge tone={tone ?? 'muted'}>
      <span className="whitespace-nowrap">{t(`assets.${group}.${value}`)}</span>
    </Badge>
  );

/**
 * The tone an in-service status carries.
 *
 * `retired` is muted rather than danger: an item leaving service is the ordinary end of its life,
 * not a fault. `under_repair` is the only one that wants attention, because it is the one state
 * somebody is expected to do something about.
 */
 ...
 *
 * `reason` names *which* permission was refused, so somebody who can read the inventory but not
 * custody learns that rather than concluding no asset has ever been issued.
 */
export const Refused = ({
  t,
  title,
  reason,
}: AssetsProps & { readonly title: string; readonly reason?: string }): ReactNode => (
  <Section title={title}>
...
export const Clear = ({
  ...
  title,
  message,
}: AssetsProps & { readonly title: string; readonly message: string }): ReactNode => (
  <Section title={title}>
 ...
 * custody row carries an employment identifier in full, two dates, a state and a day count, and at
 * 390 px those either scroll inside the table or squash the page into an unreadable column. The
 ...
 *
 * Monospaced, muted and **never shortened**. Assets holds no name for anybody — it publishes an
 * employment identifier and nothing else about the person (AD-001) — so an employment here is an
...
export const Boundaries = ({
  t,
  keys,
}: AssetsProps & { readonly keys: readonly string[] }): ReactNode => (
  <footer className="border-t border-border pt-4">
    <h2 className="text-xs font-medium uppercase tracking-wide text-muted-foreground">
      {t('assets.label.boundaries')}
    </h2>
