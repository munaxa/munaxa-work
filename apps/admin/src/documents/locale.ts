import ar from '@work/documents/locales/ar.json';
import en from '@work/documents/locales/en.json';

/**
 * The documents screens' text, in both first-class languages.
 *
 ...
 *
 * A *code* is never translated here. A document type's code and its retention policy code are
 * tenant or country-pack values, so the screen renders what the customer stored rather than
 * looking it up in a list this product ships — the same reason the domain refuses to interpret
 * them. A **status**, a **verification state**, an **expiry state** and an **access action** are
 * different: those are this module's own closed vocabularies, and an HR administrator reading
 * `download_refused` in English on an Arabic screen is a translation this product owes them.
 */
 ...
 * choice and survives review, whereas `documents.label.register` on the screen is unmistakably a
