...
import type { OnboardingRejection } from '../domain/onboarding-rejection.js';
...
export const originOfCurrentRequest = (): EventOrigin => {
  ...
  if (context === undefined || isSystemContext(context)) {
    ...
    throw new Error('An Onboarding event was raised outside a tenant context.');
  }
  return {
 ...
 * onboarding identifier confirms that somebody is being onboarded in this system, which is itself a
 * disclosure — one tenant learning another has a joiner starting on Monday.
 */
