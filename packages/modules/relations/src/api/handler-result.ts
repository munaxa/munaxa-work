 ...
 *
 * A record in another tenant is **404, not 403**. "Forbidden" on a violation identifier would
 * confirm that a disciplinary record exists for whoever it belongs to, which in this module is
 * itself the disclosure — and it would make an identifier usable as a probe.
 *
 * **No message this function produces carries a description, an employment or a person.** A refusal
 * names the resource and the rule; a rejection's text ends up in logs and error trackers, and what
 * somebody is alleged to have done does not belong in either.
 *
