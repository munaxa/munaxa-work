 ...
 *
 * **A review the caller may not see is 404, not 403.** Confirming that a review exists for a given
 * employment in a given cycle says that somebody is being appraised, which in this module is itself
 * the disclosure — and it is a disclosure a colleague can act on. The application decides this and
 * returns `not_found`; this function must not soften it into a 403 on the way out.
 *
 * **No message this function produces carries a score, a rating, a comment or an assessor's name.**
 * A refusal names the resource and the rule. A rejection's text ends up in logs and error trackers,
 * and what one person thinks of another's work does not belong in either.
 *
...
export const unwrapOrThrow = <TValue>(result: Result<TValue, HandlerFailure>): TValue => {
  ...
  switch (result.error.kind) {
    ...
    case 'forbidden':
      ...
      // on "you need performance.calibrate"; it reveals nothing about the data.
