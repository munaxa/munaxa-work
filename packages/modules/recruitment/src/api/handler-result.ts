 ...
 *
 * A record in another tenant is **404, not 403**. "Forbidden" on a candidate identifier would tell
 * the caller that identifier names a real human being in this system, which is itself a disclosure —
 * and in this module the human being never consented to being here.
 *
...
export const unwrapOrThrow = <TValue>(result: Result<TValue, HandlerFailure>): TValue => {
  ...
  switch (result.error.kind) {
    ...
    case 'forbidden':
      ...
      // "you need recruitment.hire"; it reveals nothing about the data.
