...
import type { RecruitmentRejection } from '../domain/recruitment-rejection.js';
...
export const originOfCurrentRequest = (): EventOrigin => {
  ...
  if (context === undefined || isSystemContext(context)) {
    ...
    throw new Error('An Recruitment event was raised outside a tenant context.');
  }
  return {
 ...
 * actor could record a requisition approval as though a colleague had committed the headcount — and
 * an approval somebody can forge is not evidence of anything.
 */
...
export const refusedBy = <TValue>(
  rejection: RecruitmentRejection,
 ...
 * candidate identifier tells the caller that identifier names a real human being in this system,
 * which is itself a disclosure — and in this module the human being never consented to being here.
 */
