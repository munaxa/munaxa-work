 ...
 *
 * This supplies the rest — the audit columns and the parameter placeholders — so eleven repositories
 * do not each write `created_at, created_by, updated_at, updated_by, version` and each get one
 * chance to omit one.
 *
 * It is a near-copy of Employment's file rather than an import of it: a module may not reach into
 ...
 * applied date read on a server west of UTC comes back as the previous day — which moves an offer's
 * start date and, at hire, the date an employment begins. Every date column in this module is
 * selected with `to_char(..., 'YYYY-MM-DD')` so the value that leaves the database is the value that
 * was stored.
 *
 * The alias is separate from the expression because a qualified column (`a.applied_on`) cannot be
 * its own alias — `as a.applied_on` is a syntax error, and the one that reads as a mystery.
 */
