...
import type { EmploymentRejection } from '../domain/employment-rejection.js';
...
export const originOfCurrentRequest = (): EventOrigin => {
  ...
  if (context === undefined || isSystemContext(context)) {
    ...
    throw new Error('An Employment event was raised outside a tenant context.');
  }
  return {
 ...
 * actor could record a suspension as though a colleague had ordered it — and a status history
 ...
 */
export const refusedBy = <TValue>(rejection: EmploymentRejection): Result<TValue, HandlerFailure> =>
  rejected(rejection.messageKey);

/**
 * Nothing found, for a caller who has no business learning otherwise.
 *
 * Used for an employment in another tenant as well as one that does not exist. "Forbidden" on an
 * employment identifier tells the caller that identifier names a real employment in this system,
