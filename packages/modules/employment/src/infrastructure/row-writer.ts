 ...
 *
 * This supplies the rest — the audit columns and the parameter placeholders — so six repositories
 * do not each write `created_at, created_by, updated_at, updated_by, deleted_at, deleted_by,
 * version` and each get one chance to omit one.
 */
...
/** `null` in a column, `undefined` in a domain state. Converted here rather than at six sites. */
 ...
 * a start date read on a server west of UTC comes back as the previous day — which shifts a
 * probation end, a notice period and, in several of this product's markets, an end-of-service
 * entitlement. Every date column in this module is selected with `to_char(..., 'YYYY-MM-DD')` so
 * the value that leaves the database is the value that was stored.
 *
 * The alias is separate from the expression because a qualified column (`e.start_date`) cannot be
 * its own alias — `as e.start_date` is a syntax error, and the one that reads as a mystery.
 */
export const civilDateColumn = (column: string, alias?: string): string =>
  `to_char(${column}, 'YYYY-MM-DD') as ${alias ?? column}`;
