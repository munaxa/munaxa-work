...
import { postgresEmploymentStores } from './employment-stores.js';
 ...
 * row-level security that isolates six tables of workforce data, the partial unique indexes that
 * keep one open employment per person and one open primary assignment per employment, the check
 * constraints that refuse an undated termination, and the counter that two concurrent creates
 * serialize on. A mock would only prove the mock behaves as instructed.
 *
...
/**
 * Employment references Person by foreign key, so the fixture seeds one to point at.
 *
...
export interface EmploymentFixture {
  readonly admin: Pool;
  readonly application: Pool;
  readonly unitOfWork: UnitOfWork;
  readonly stores: ReturnType<typeof postgresEmploymentStores>;
  asTenant<TResult>(
    tenantId: string,
    work: (transaction: Transaction) => Promise<TResult>,
  ): Promise<TResult>;
  seedPerson(tenantId: string, personNumber: string): Promise<string>;
 ...
 * missing migration step. Phase 2 learned this the expensive way.
 */
const assertSchemaApplied = async (admin: Pool): Promise<void> => {
  const expected = [...EMPLOYMENT_TABLES, PERSON_TABLE];
  ...
  if (missing.length > 0) {
    throw new Error(
      `Employment's tables are not in this database: ${missing.join(', ')}. ` +
...
const ensureApplicationRole = async (admin: Pool, role: string): Promise<string> => {
  ...
  await admin.query(
    `grant select, insert, update, delete on ${[...EMPLOYMENT_TABLES, PERSON_TABLE].join(', ')} to ${role}`,
  );
 ...
 * failure until the job's own timeout hours later.
 */
const BOUNDS = {
...
const seedPersonWith = async (
  ...
  await admin.query(
    ...
    // Suffixed with the identifier, because these rows outlive the test that made them: `person`
    // is not truncated here, and People's unique index would refuse a second `P-1`.
    [id, tenantId, `${personNumber}-${id.slice(-12)}`],
  );
  return id;
};
 ...
 */
export const openEmploymentFixture = async (role: string): Promise<EmploymentFixture> => {
  ...
  const stores = postgresEmploymentStores();
  ...
  return {
    ...
    seedPerson: (tenantId, personNumber) => seedPersonWith(admin, tenantId, personNumber),
    // Only this module's tables. `person` is People's, and emptying it here would delete rows a
    // concurrently running People suite is asserting on — and, through the foreign key, cascade
    // into employments this suite had just written.
    truncate: async () => {
      await admin.query(`truncate ${EMPLOYMENT_TABLES.join(', ')} cascade`);
    },
    /**
