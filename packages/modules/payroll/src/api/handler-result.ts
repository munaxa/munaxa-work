 ...
 *
 * A record in another tenant is **404, not 403**. "Forbidden" on a compensation identifier would
 * confirm that somebody in this system is paid something, which is itself a disclosure.
 *
 * **No message this function produces carries a monetary amount.** A refusal names the field, the
 * run or the date; it says net pay would have been negative and not by how much. A rejection's text ends up in logs and error
 * trackers, and a salary does not belong in either.
 *
...
export const unwrapOrThrow = <TValue>(result: Result<TValue, HandlerFailure>): TValue => {
  ...
  switch (result.error.kind) {
    ...
    case 'forbidden':
      ...
      // on "you need payroll.finalize"; it reveals nothing about the data.
