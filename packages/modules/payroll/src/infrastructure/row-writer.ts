 ...
 *
 * A near-copy of Compensation's, Leave's, Attendance's, Onboarding's and Recruitment's file rather
 * than an import of one: a module may not reach into another module's internals, and hoisting it
 * into `@work/persistence` is a change to a package every phase depends on, recorded as debt rather
 * than made inside a business phase. **This is the sixth copy**, and the debt is restated in the
 * Phase 11 report.
 *
 * `asBigInt` matters more here than anywhere before it. The driver returns `bigint` columns as
 * **strings** precisely so nothing above 2^53 is silently mangled, and this module depends on that:
 * there is no path from a database row to a monetary amount that passes through `number`.
 */
 ...
 * matters more than in any other — a payroll query that lost its tenant clause would put one
 * customer's net pay in front of another's.
 */
