...
import { postgresLeaveStores } from './leave-stores.js';
 ...
 * security that isolates fourteen tables, the **GiST exclusion constraint** that settles a race
 * between two requests for the same morning, the unique indexes that make every bounded run
 * idempotent, the partial index the reconciliation read depends on, the foreign keys that make it
 * impossible for Leave to invent an employment, and the check constraint that refuses a
 * self-approved request. A mock would only prove the mock behaves as instructed — and the
 * properties this module's correctness rests on are precisely the ones only a real database shows.
 *
...
/**
 * Leave references an Employment by foreign key, so the fixture seeds one — and seeds the Person
 * the employment itself requires.
 *
 ...
 * shape of the dependency: Leave creates neither, in this fixture or in production (ADR-0051).
 */
const FOREIGN_TABLES = ['person', 'employment'];

export interface LeaveFixture {
  readonly admin: Pool;
  readonly application: Pool;
  readonly unitOfWork: UnitOfWork;
  readonly stores: ReturnType<typeof postgresLeaveStores>;
  asTenant<TResult>(
    tenantId: string,
...
const assertSchemaApplied = async (admin: Pool): Promise<void> => {
  const expected = [...LEAVE_TABLES, ...FOREIGN_TABLES];
  ...
  if (missing.length > 0) {
    throw new Error(
      `Leave's tables are not in this database: ${missing.join(', ')}. ` +
...
const ensureApplicationRole = async (admin: Pool, role: string): Promise<string> => {
  ...
  await admin.query(
    `grant select, insert, update, delete on ${[...LEAVE_TABLES, ...FOREIGN_TABLES].join(', ')} to ${role}`,
  );
...
const seedEmploymentWith = async (
  ...
  await admin.query(
    ...
    [personId, tenantId, `LVE-${personId.slice(-12)}`],
  );
  await admin.query(
    ...
    [employmentId, tenantId, personId, `LVE-${employmentId.slice(-12)}`, startDate],
  );
  return employmentId;
};
 ...
 */
export const openLeaveFixture = async (role: string): Promise<LeaveFixture> => {
  ...
  const stores = postgresLeaveStores();
  ...
  return {
    ...
    truncate: async () => {
      await admin.query(`truncate ${LEAVE_TABLES.join(', ')} cascade`);
    },
    /**
