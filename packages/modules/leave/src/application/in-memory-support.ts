...
import type { Page } from './leave-ports.js';
 ...
 *
 * They reproduce the **unique indexes that matter**, and they raise the same SQLSTATE the driver
 * would, because several code paths branch on that error and a fake that failed differently would
 * leave those branches untested until production found them.
 *
 * They are **not a substitute for the integration suites**. Row-level security, the GiST exclusion
 * constraint, the check constraints and the real indexes belong to the database, and only a real
 * one can prove them.
 */
...
/** The error the GiST exclusion constraint raises when two requests claim the same minutes. */
...
/** Whether two date ranges touch at all. Used where the SQL would use an overlap operator. */
export const rangesOverlap = (
  one: { readonly from: string; readonly to: string },
  other: { readonly from: string; readonly to: string },
): boolean => one.from <= other.to && other.from <= one.to;
