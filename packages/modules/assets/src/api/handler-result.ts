 ...
 *
 * A record in another tenant is **404, not 403**, so an identifier cannot be used as a probe: a
 * caller who guesses one learns exactly what a caller who invents one learns.
 *
...
export const unwrapOrThrow = <TValue>(result: Result<TValue, HandlerFailure>): TValue => {
  ...
  switch (result.error.kind) {
    ...
    case 'forbidden':
      ...
      // "you need assets.asset.manage"; it reveals nothing about the data.
