...
import { postgresAssetsStores } from './assets-stores.js';
 ...
 * under which any isolation assertion means anything. A suite run as a superuser would report that
 * one tenant cannot read another's inventory **without having checked**.
 */
...
export interface AssetsFixture {
  ...
  readonly stores: ReturnType<typeof postgresAssetsStores>;
  asTenant<TResult>(
    tenantId: string,
    work: (transaction: Transaction) => Promise<TResult>,
  ): Promise<TResult>;
  /** Runs as a *named* actor, for the assertions about who created and who last amended a row. */
  asActor<TResult>(
    tenantId: string,
    actor: string,
    work: (transaction: Transaction) => Promise<TResult>,
...
const assertSchemaApplied = async (admin: Pool): Promise<void> => {
  ...
  const missing = ASSETS_TABLES.filter(
    (table) => !present.rows.some((row) => row.table_name === table),
  );

  if (missing.length > 0) {
    throw new Error(
      `Assets' tables are not in this database: ${missing.join(', ')}. ` +
        'These suites exercise the real schema — its policies, indexes and check constraints — so ' +
        'run `pnpm db:migrate` against TEST_DATABASE_URL first.',
    );
  }
};
...
const ensureApplicationRole = async (admin: Pool, role: string): Promise<string> => {
  ...
  await admin.query(
    `grant select, insert, update, delete on ${ASSETS_TABLES.join(', ')} to ${role}`,
  );
 ...
 */
export const openAssetsFixture = async (role: string): Promise<AssetsFixture> => {
  ...
  const stores = postgresAssetsStores();
