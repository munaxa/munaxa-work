...
import { postgresCompensationStores } from './compensation-stores.js';
 ...
 * security that isolates fourteen tables holding the most sensitive data in this product, the
 * **GiST exclusion constraint** that settles a race between two administrators assigning the same
 * component, the unique indexes that make an import idempotent, the check constraint that refuses a
 * self-approved change, the ordering constraint on a pay grade, and the exactness of a `bigint`
 * amount above 2^53 through a driver round-trip. A mock would only prove the mock behaves as
 * instructed — and the properties this module's correctness rests on are precisely the ones only a
 * real database shows.
 *
...
export const TENANT_A = '01930000-0000-7000-8000-0000000c1111';
export const TENANT_B = '01930000-0000-7000-8000-0000000c2222';

/** The fourteen tables this module owns, most dependent first, for truncation between tests. */
export const COMPENSATION_TABLES = [
  'compensation_change',
...
/**
 * Compensation references an Employment by foreign key, so the fixture seeds one — and seeds the
 ...
 * shape of the dependency: Compensation creates neither, in this fixture or in production
 * (ADR-0051).
 */
...
export interface CompensationFixture {
  ...
  readonly stores: ReturnType<typeof postgresCompensationStores>;
  asTenant<TResult>(
    tenantId: string,
...
const assertSchemaApplied = async (admin: Pool): Promise<void> => {
  const expected = [...COMPENSATION_TABLES, ...FOREIGN_TABLES];
  ...
  if (missing.length > 0) {
    throw new Error(
      `Compensation's tables are not in this database: ${missing.join(', ')}. ` +
 ...
 * one tenant cannot read another's salaries without having checked.
 */
const ensureApplicationRole = async (admin: Pool, role: string): Promise<string> => {
  ...
  await admin.query(
    `grant select, insert, update, delete on ${[...COMPENSATION_TABLES, ...FOREIGN_TABLES].join(', ')} to ${role}`,
  );
...
const seedEmploymentWith = async (
  ...
  await admin.query(
    ...
    [personId, tenantId, `CMP-${personId.slice(-12)}`],
  );
  await admin.query(
    ...
    [employmentId, tenantId, personId, `CMP-${employmentId.slice(-12)}`, startDate],
  );
  return employmentId;
};
 ...
 */
export const openCompensationFixture = async (role: string): Promise<CompensationFixture> => {
  ...
  const stores = postgresCompensationStores();
  ...
  return {
    ...
    truncate: async () => {
      await admin.query(`truncate ${COMPENSATION_TABLES.join(', ')} cascade`);
    },
    /**
