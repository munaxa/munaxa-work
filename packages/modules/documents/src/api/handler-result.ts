 ...
 *
 * A record in another tenant is **404, not 403**, and so is a confidential document the caller may
 * not see. "Forbidden" on a document identifier would confirm that a document of that kind exists
 * for that employee, which in this module is itself the disclosure — a manager learning that a
 * medical certificate exists has learned the thing the confidentiality level was protecting.
 *
 * **No message this function produces carries a file name, a storage reference or a URL.** A
 * refusal names the resource and the rule; a rejection's text ends up in logs and error trackers,
 * and the address of somebody's passport scan does not belong in either.
 *
...
export const unwrapOrThrow = <TValue>(result: Result<TValue, HandlerFailure>): TValue => {
  ...
  switch (result.error.kind) {
    ...
    case 'forbidden':
      ...
      // on "you need document.verify"; it reveals nothing about the data.
